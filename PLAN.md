...
- **当前阶段**：独立硬件适配分支上的 EV-04 非阻塞音频流水线。固定采播槽池、
  独立任务、取消代际隔离和静态编译已完成，等待目标板连接后补真实停声时延及
  运行时资源数据。软件 `SW-GATE` 环境验收仍待完成，暂不合并 `main`，不启动第 12 步。
...
当前 HAL 和 EV-04 非阻塞音频实现、静态编译及主机回归已完成，但目标板运行时
资源和停声时延实测仍缺失。用户已于 2026-09-17 明确将 EV-04 代码实现提前，
该决定不豁免 EV-03、EV-04 的实物验收或软件 `SW-GATE`。
